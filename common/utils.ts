 ...
 * Used for configuring requests to the GitHub API
 */
type RequestOptions = {
  method?: string;
  body?: unknown;
 ...
 * 
 * Authentication is handled via GITHUB_PERSONAL_ACCESS_TOKEN environment variable
 ...
 * 4. Copy token and add to .env file as GITHUB_PERSONAL_ACCESS_TOKEN=your_token_here
 * 
 * @param url The GitHub API endpoint URL
 * @param options Request options (method, body, headers)
 * @returns The parsed response from GitHub API
 */
export async function githubRequest(
  ...
  if (process.env.GITHUB_PERSONAL_ACCESS_TOKEN) {
    ...
    console.warn("Warning: GITHUB_PERSONAL_ACCESS_TOKEN not found in environment variables. API requests may be rate-limited or fail.");
  }
