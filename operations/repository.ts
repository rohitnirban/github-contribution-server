...
// New schema for daily contribution operation
export const CreateDailyContributionSchema = z.object({
  prefix: z.string().optional().describe("Optional prefix for repository name (default: 'daily-contribution')"),
});

// Type exports
export type CreateRepositoryOptions = z.infer<typeof CreateRepositoryOptionsSchema>;
export type CreateDailyContributionOptions = z.infer<typeof CreateDailyContributionSchema>;
...
/**
 * Creates a private repository for daily GitHub contribution
 * The repository will be created with a README.md file 
 * and will be named based on the current date
 * 
 * @param options Optional configuration options
 * @returns The created repository information
 */
export async function createDailyContribution(options?: CreateDailyContributionOptions) {
  // Get today's date in YYYY-MM-DD format
  const today = new Date().toISOString().split('T')[0];
  const prefix = options?.prefix || 'daily-contribution';
  
  // Create the repository with today's date in the name
  const repoName = `${prefix}-${today}`;
  const description = `Daily GitHub contribution for ${today}`;
  
  // Create repository with README initialized and set to private
  const response = await githubRequest("https://api.github.com/user/repos", {
    method: "POST",
    body: {
      name: repoName,
